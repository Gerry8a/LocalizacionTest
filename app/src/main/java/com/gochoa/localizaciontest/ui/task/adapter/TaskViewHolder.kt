...
class TaskViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    ...
    fun render(task: TaskEntity, onItemSelected: (TaskEntity) -> Unit) {
        ...
        when (task.status) {
            "Por hacer" -> {
                binding.cvTask.setCardBackgroundColor(ContextCompat.getColor(context, R.color.md_theme_light_inversePrimary))
            }
            "En progreso" ->{
                binding.cvTask.setCardBackgroundColor(ContextCompat.getColor(context, R.color.progress))
            }
            "Terminada" ->{
                binding.cvTask.setCardBackgroundColor(ContextCompat.getColor(context, R.color.disable))
            }
        }
    }
}